...
#THIS WAS THE FILE USED IN THE VIRTUAL MACHINE FOR TESTING AND DEVELOPMENT
...
def process_images(image_paths):
    ...
    for image_path in image_paths:
        ...
        if results.boxes.data.size(0) > 0:
            for result in results.boxes.data.tolist():
                x1, y1, x2, y2, score, class_id = result
                print(f"Detections: {results.names[int(class_id)]} with score {score}")
                if score > threshold:
                    detected = True
            ...
            print(f"No valid detections found for {image_path}")
        delete_image(image_path)
