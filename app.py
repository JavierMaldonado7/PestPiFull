...
def get_filt():
    ...
    if location_filter != 'all':
        query = query.join(PestPi).filter(PestPi.pi_location == location_filter)

    results = query.all()
    alerts = [{
        'alert_id': alert.alert_id,
        'alert_type': alert.alert_type,
        'alert_date': alert.alert_date.strftime('%Y-%m-%d %H:%M'),  # Adjusted format
        'alert_location': get_pi_location(alert.pi_id)  # Ensure you have a mechanism to get location
